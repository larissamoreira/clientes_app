...
if(cepinfo) {
    cepinfo.addEventListener('blur', function() {
        ...
        let url2 = `https://maps.googleapis.com/maps/api/geocode/json?address=rua+coronel+aristarco+pessoa+741,joao+pessoa&key=AIzaSyB2uPawOPYriT7y7vdd0OeNp_JButueuvE`
            fetch(url2)
                .then(response => response.json())
                .then(data => {
